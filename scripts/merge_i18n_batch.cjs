...
// Allow passing a different additions filename as the first CLI argument
// or via I18N_ADDITIONS_FILE env var. If not provided, fall back to batch_1_additions.json
const defaultAdditionsFile = path.join(localesDir, 'batch_1_additions.json');
let additionsFile = defaultAdditionsFile;
if (process.argv[2]) {
  // allow either a basename (resolved relative to localesDir) or an absolute path
  const maybe = process.argv[2];
  additionsFile = path.isAbsolute(maybe) ? maybe : path.join(localesDir, maybe);
} else if (process.env.I18N_ADDITIONS_FILE) {
  const maybe = process.env.I18N_ADDITIONS_FILE;
  additionsFile = path.isAbsolute(maybe) ? maybe : path.join(localesDir, maybe);
}
