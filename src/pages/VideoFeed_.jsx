...
import { Box, Typography, IconButton, Avatar, Chip, LinearProgress } from '@mui/material';
...
const SwipeIndicator = styled(Box)(({ theme, direction }) => ({
  ...
  alignItems: 'center',
  backgroundColor: direction === 'right' ? 'rgba(76, 175, 80, 0.3)' : 'rgba(244, 67, 54, 0.3)',
  zIndex: 4,
  opacity: 0,
...
const VideoFeed = () => {
  ...
  const handleSwipe = async (direction) => {
    ...
    try {
      ...
      // انتقل إلى الوظيفة التالية بعد تسجيل السحب
      setTimeout(() => {
  ...
  if (jobs.length === 0) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 'calc(100vh - 56px)' }}>
        <Typography variant="h6">لا توجد وظائف متاحة حالياً</Typography>
      </Box>
    );
  }
  ...
  if (currentIndex >= jobs.length) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 'calc(100vh - 56px)' }}>
        <Typography variant="h6">لقد وصلت إلى نهاية القائمة</Typography>
      </Box>
    );
  }
  ...
  return (
    <Box sx={{ position: 'relative', height: 'calc(100vh - 56px)' }}>
      <VideoContainer {...swipeHandlers}>
        <VideoElement 
          src={currentJob.video_url || "https://example.com/placeholder-video.mp4"} 
          autoPlay 
          loop 
          muted 
          playsInline
        ...
        <VideoOverlay>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
            ...
            <Typography variant="body2">{currentJob.employment_type}</Typography>
          ...
          <Box sx={{ mt: 1 }}>
            {currentJob.skills && currentJob.skills.map(skill => (
        ...
        <ActionButtons>
          ...
          <CircleButton aria-label="حفظ">
            <BookmarkIcon />
          </CircleButton>
          
          <CircleButton aria-label="مشاركة">
            <ShareIcon />
          </CircleButton>
        </ActionButtons>
