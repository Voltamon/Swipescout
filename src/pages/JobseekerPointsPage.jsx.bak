    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      {loading ? <div>Loading…</div> : error ? <div className="text-red-500">{error}</div> : (
        <>
          ...
          <div>
            <h2 className="text-xl font-semibold mt-6 mb-2">Redeem</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {conversions.map((c) => (
                <Card key={`${c.type}:${c.key}`}>
                  <CardContent>
                    ...
                    <button className="mt-3 px-3 py-1 rounded bg-blue-600 text-white" onClick={() => handleRedeem(c.key, c.type)}>Redeem</button>
                  </CardContent>
                </Card>
              ))}
            </div>
